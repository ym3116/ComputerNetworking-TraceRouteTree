...
def trace():
    ...
    # Parse targets from uploaded CSV
    content = file.read().decode('utf-8')
    reader = csv.reader(io.StringIO(content))
    next(reader, None)  # skip header
    targets = [row[0] for row in reader if row]  # assumes IP is in column 0

    # Reset progress before starting
    progress_status["progress"] = 0

    # Run traceroute
    results = run_traceroute(targets)

    return jsonify(results)

@app.route('/api/progress', methods=['GET'])
def progress():
    return jsonify({"percentage": progress_status["progress"]})
